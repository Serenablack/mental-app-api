...
public class AuthController {
    ...
    public ResponseEntity<UserDto.AuthResponse> registerUser(
        ...
        try {
            ...
            return ResponseEntity.ok(new UserDto.AuthResponse());
        } catch (Exception e) {
            log.error("Registration failed for username: {}", request.getUsername(), e);

            return ResponseEntity.badRequest().body(new UserDto.AuthResponse("Registration failed: " + e.getMessage()));
        }
    }
     ...
     */
    @PostMapping("/login")
    public ResponseEntity<UserDto.AuthResponse> loginUser(@Valid @RequestBody UserDto.UserLoginRequest request) {
        log.info("Login request received for user: {}", request.getUsernameOrEmail());

        try {
            UserDto.AuthResponse response = userService.authenticateUser(request);
            ...
            return ResponseEntity.badRequest().body(new UserDto.AuthResponse("Login failed: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<UserDto.AuthResponse> googleAuthCallback(
            @Valid @RequestBody UserDto.GoogleAuthRequest request) {
    ...
    public ResponseEntity<UserDto.AuthResponse> verifyEmail(
